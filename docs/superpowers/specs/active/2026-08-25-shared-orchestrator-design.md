...
## Implementation Note (added 2026-08-25, before planning)

This repo currently has **no** `apps/lhr-office`, no `@lhr/db`, and none of the five agent
pipelines this spec assumes already exist — it's a single root-level Astro site plus `mcp-server`
and `packages/schemas` as its only workspace members. The five agent specs referenced in the
opening paragraph were never found in this repository (checked `docs/superpowers/specs/`, full
git history, all branches).

Decision (confirmed with the author before writing the implementation plan): the plan builds
`apps/lhr-office` as a new Vercel project/workspace member (monorepo-style, alongside the existing
`mcp-server` and `packages/schemas`, not by relocating the existing production site), plus
`packages/db` and `packages/jobs`, implementing the full generic orchestrator engine — registry
contract, due-check, overlap guard, data model, cron endpoint, `/status` page, manual run-now.
`packages/jobs/src/registry.ts` ships with **zero entries**. Each of the five agents gets appended
to that registry in its own future implementation plan, once its pipeline spec exists — exactly
the "adding a sixth agent is registering one more entry" property this spec already asks for in
§1, just starting from zero instead of five.
