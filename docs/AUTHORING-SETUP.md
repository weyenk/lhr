...
**Known follow-up — `mcp-server/src/blob.ts`'s photo uploads still request `access: 'public'`** against this same (private) store, so publishing a post with a photo will hit the identical `BlobError` until that's addressed. Fixing it needs a store that's actually public — e.g. a second Vercel Blob store provisioned specifically for photos — since post photos must be readable by anonymous site visitors, which a private store's token-gated reads can't support.
