...
**Resolved — sibling-workspace installs were being scoped out, not cached out.** After `packages/jobs`, `packages/db`, and `apps/lhr-office` were added, both `mcp-server` and `apps/lhr-office` (each a separate Vercel project whose Root Directory is a subdirectory) started failing the root `postinstall` script — `mcp-server` on `Cannot find module 'pg'` building `@lhr/db`, `apps/lhr-office` on `Cannot find module 'zod'` building `@lhr/schemas`. An earlier fix moved `@types/pg` into `@lhr/db`'s `dependencies` on the theory that Vercel's *cached* install was dropping a devDependency-only type package; that only masked the symptom once. The real cause: when a Vercel project's Root Directory is a subdirectory, Vercel installs only that project's own dependency closure, not the whole workspace — `mcp-server` never declares `@lhr/db` as a dependency, so `pg` never gets installed for it, and likewise `apps/lhr-office` never declares `@lhr/schemas`, so `zod` never gets installed for it — yet the root `postinstall` unconditionally builds all three internal packages regardless of which project triggered the install. Fix: both `mcp-server/vercel.json` and `apps/lhr-office/vercel.json` now set `installCommand` to `cd` up to the true repo root before running `npm install`, forcing the same full, unscoped workspace install the main site's project already gets for free (its Root Directory *is* the repo root).
