...
**Note on step 2's project root:** `mcp-server/src/tools/confirmAndPublish.ts` imports the site's post/set/product schemas directly from `../../../src/content/schemas` (outside `mcp-server/`), so it can validate a draft against the real site schema before publishing. This works in every local build and test run, but has not been confirmed against Vercel's actual serverless bundling with the project root set to `mcp-server/` (step 2) — if Vercel's dependency tracing doesn't include files above the configured root, `confirm_and_publish` will fail at runtime with a module-not-found error. Verify this specifically on first real deploy (call `confirm_and_publish` once and confirm it succeeds); if it fails this way, the schemas may need to be duplicated into or symlinked within `mcp-server/`, or the Vercel project's root directory reconsidered.
