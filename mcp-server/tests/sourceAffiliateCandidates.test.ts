...
// The shared orchestrator pool: a process-wide singleton this job borrows and must never end().
...
describe('sourceAffiliateCandidates', () => {
  it('returns failure when Keepa fails, without ever calling selectCycle', async () => {
  ...
  it('throws if a required env var is missing, before touching the shared pool', async () => {
  ...
  it('borrows the shared orchestrator pool and never ends it', async () => {
    keepaMock.findTrendingCandidates.mockResolvedValue([]);
    await sourceAffiliateCandidates();
    expect(dbMock.getPool).toHaveBeenCalled();
    expect(dbMock.getDecidedAsins).toHaveBeenCalledWith(mockDb);
  });
